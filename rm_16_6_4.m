% [[16,6,4:2]] Quantum RM code which can correct single X-type data and
% measurement errors and upto 3-data and measuremnt errors of Z type
r = 2;
m = 4;
[H_1, points] = kasami_grm_paritycheck_kasami(r, m);
H_2 = H_1;
[m_1,n] = size(H_1);
m_2 = m_1;
...
[h1_per, is_codewordx] = stack_left_shifts_in_code(H_1, m,1); 
[h2_per, is_codewordz] = stack_left_shifts_in_code(H_2, m,1); 
r_1 = size(h1_per,1);
r_2 = size(h2_per,1);

% Adding the all one vector 
H_1 = [ones(1,n);H_1];
h1_per(1,:) = mod(ones(1,n)+h1_per(1,:),2);
